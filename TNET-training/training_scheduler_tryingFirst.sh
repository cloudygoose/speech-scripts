...
for iter in $(seq -w $MAX_ITER); do
  ...
  echo STILL_FINDING=$STILL_FINDING
  ...
  if [[ 1 == $(awk 'BEGIN{print('$accu_cv'<'$accu_prev'+'$START_HALVING_INC')}') && $STILL_FINDING == 0 ]]; then
    do_halving=1
  fi

  if [[ $do_halving == 1 || $STILL_FINDING == 1 ]]; then
